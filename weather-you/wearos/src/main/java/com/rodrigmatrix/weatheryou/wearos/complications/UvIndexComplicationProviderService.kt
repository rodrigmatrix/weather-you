...
class UvIndexComplicationProviderService : SuspendingComplicationDataSourceService(), KoinComponent {
        ...
        return when (type) {
            ComplicationType.RANGED_VALUE -> {
                RangedValueComplicationData.Builder(
                    value = currentLocation.uvIndex.toFloat(),
                    min = 0f,
                    max = 10f,
                    contentDescription = PlainComplicationText.Builder(
                        text = currentLocation.uvIndex.roundToInt().toString()
                    ).build()
                )
                    .setColorRamp(
                        ColorRamp(
                    ...
                    .setMonochromaticImage(
                        MonochromaticImage.Builder(
                            image = createWithResource(
                                this,
                                com.rodrigmatrix.weatheryou.weathericons.R.drawable.ic_sunny,
                            )
                        ).build()
                    )
                    .setText(
                    ...
                    .setTitle(
                        PlainComplicationText.Builder(
                            text = getString(R.string.uv_index),
                        ).build()
                    )
